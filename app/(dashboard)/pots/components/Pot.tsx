    ...
    <section className="min-h-[303px] bg-white rounded-xl py-8 px-6 xl:px-10">
      ...
      <div className="bg-[#F8F4F0] h-[8px] rounded-md w-full mt-6">
        <div
          ...
          style={{
            width: `calc(${total >= target ? 100 : calculateExpensesInPercentage}%)`,
      ...
      <div className="flex items-center justify-between gap-3 w-full mt-10">
          <Button className="flex-grow py-7 bg-[#F8F4F0] hover:bg-[#F8F4F0] text-[#201F24] font-semibold text-base shadow-none"><PlusIcon color="#000" size={14} /> Add Money</Button>
          <Button className="flex-grow py-7 bg-[#F8F4F0] hover:bg-[#F8F4F0] text-[#201F24] font-semibold text-base shadow-none" onClick={() => setIsOpenWithdrawModal(prev => !prev)}>Withdraw</Button>
      </div>
    </section>

    <EditPot isOpen={isOpen} loading={false} onClose={() => setIsOpen(prev => !prev)} onConfirm={() => setIsOpen(prev => !prev)} name={name} target={target} theme={theme} />

    <WithdrawPotModal isOpen={isOpenWithdrawModal} loading={false} onClose={() => setIsOpenWithdrawModal(prev => !prev)} onConfirm={() => setIsOpenWithdrawModal(prev => !prev)} name={name} total={total} target={target} />

      <DeletePotModal isOpen={isOpenDeleteModal} loading={false} onClose={() => setIsOpenDeleteModal(prev => !prev)} onConfirm={() => setIsOpenDeleteModal(prev => !prev)} name={name} />
    </>
  );
};
